...
fn get_input_iterator(input: &str) -> impl Iterator<Item = Result<usize, failure::Error>> + '_ {
    input.split_whitespace().map(|x| Ok(x.parse()?))
}

fn get_node_sum(
    iter: &mut impl Iterator<Item = Result<usize, failure::Error>>,
) -> Result<usize, failure::Error> {
    let child_nodes = iter
        .next()
        .unwrap_or_else(|| bail!("Missing child nodes quantity"))?;
    let metadata = iter.next().unwrap_or_else(|| bail!("Missing metadata"))?;
    let mut sum = 0;
    for _ in 0..child_nodes {
        sum += get_node_sum(iter)?;
    }
    for _ in 0..metadata {
        sum += iter.next().unwrap_or_else(|| bail!("Missing metadata"))?;
    }
    Ok(sum)
}
...
fn get_root_node_value(
    ...
    let mut sum = 0;
    for _ in 0..metadata {
        let value = iter.next().unwrap_or_else(|| bail!("Missing metadata"))?;
